...
def identity_block(input_tensor, kernel_size, filters, stage, block, strides = (1,1)):
    ...
    x = Conv2D(filters1, kernel_size, padding = 'same', strides = strides, name=conv_name_base + '2a')(input_tensor)   
    x = BatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)
 
    x = Conv2D(filters2, kernel_size , strides = strides, padding='same', name=conv_name_base + '2b')(x)
...
def ResNet34(include_top = False, 
            weights=None,
    ...
    if include_top:
        x = Flatten()(x)
        x = Dense(classes, activation='softmax', name='resnet34')(x)
    else:
        if pooling == 'avg':
